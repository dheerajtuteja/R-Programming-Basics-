...
#Round
revenue.1000  <- round(revenue,2)/1000
expenses.1000 <-round(expenses,2)/1000

#  - profit for each month
PoM <- round(revenue.1000 -expenses.1000,2)
PoM
...
profit.margin <- round(PoMNoT / revenue.1000,2)*100
profit.margin

#Average Profit Net of Taxes
APNoT <- mean(PoLNoT)
round(APNoT,2)

#- good months
good.months <- profit.margin > APNoT
good.months
...
#- the best month
best.month <- profit.margin == max(profit.margin)
best.month

#- the worst month
worst.month <- profit.margin == min(profit.margin)
worst.month